...
void MCP_CAN::mcp2515_read_canMsg( const INT8U buffer_sidh_addr)        /* read can msg                 */
{
	...
	// this will be the address of the message buffer
    mcp_addr = buffer_sidh_addr;

	// reads the ID of the message in the buffer, including extended frame flag.
    mcp2515_read_id( mcp_addr, &m_nExtFlg,&m_nID );		
	// reads register for RTR bit
    ctrl = mcp2515_readRegister( mcp_addr-1 );

	// reads data length code
    m_nDlc = mcp2515_readRegister( mcp_addr+4 );

	// checks if RTR bit is set
    if ((ctrl & 0x08)) {
        m_nRtr = 1;
    }
    else {
    ...
    m_nDlc &= MCP_DLC_MASK;
	// reads the registers to obtain the actual message
...
// ulData corresponds to the actual mask. For standard messaging, it is the first 11 bits and then the
// first 17 bits of the data. For extended, it is the identifier.
// 
//MCP_CAN::init_Mask(INT8U num, INT8U ext, INT32U ulData)
//{
...
INT8U MCP_CAN::sendMsgBufPri(INT32U id, INT8U ext, INT8U len, INT8U* buf, INT8U pri)
{
    setMsg(id, ext, len, buf);
    //sendMsg();
    INT8U res, res1, txbuf_n;
    ...
    uiTimeOut = 0;
    mcp2515_write_canMsg(txbuf_n);
    // this function needs changed to set a priority
    mcp2515_start_transmit(txbuf_n);
    do
    {
        uiTimeOut++;
...
INT8U MCP_CAN::readMsg()
{
	...
	// checks which receive buffer the message is in
    if ( stat & MCP_STAT_RX0IF )                                        /* Msg in Buffer 0              */
    {
        mcp2515_read_canMsg( MCP_RXBUF_0);								/* Reads the message */
        mcp2515_modifyRegister(MCP_CANINTF, MCP_RX0IF, 0);				/* Clears buffer flag so new message can be sent */
        res = CAN_OK;
    }
